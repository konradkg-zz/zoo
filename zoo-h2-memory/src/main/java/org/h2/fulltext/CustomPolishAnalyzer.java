...
public class CustomPolishAnalyzer extends StopwordAnalyzerBase {
	...
	static {
	    ...
	    try {
		DEFAULT_TABLE = StempelStemmer.load(PolishAnalyzer.class
		        .getResourceAsStream(DEFAULT_STEMMER_FILE));
	    } catch (IOException ex) {
		// default set should always be present as it is part of the
		// distribution (JAR)
     ...
     */
    public CustomPolishAnalyzer(Version matchVersion) {
     ...
     *            a stopword set
     */
    public CustomPolishAnalyzer(Version matchVersion, Set<?> stopwords) {
     ...
     */
    public CustomPolishAnalyzer(Version matchVersion, Set<?> stopwords, Set<?> stemExclusionSet) {
     ...
     *         exclusion set is provided and {@link StempelFilter}.
     */
    @Override
